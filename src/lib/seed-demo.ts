 ...
 *
 * The account is created as a password user AND can be reached via Google
 * OAuth (the callback links the Google identity by email). The demo password
 * is (re)set on every seed so email+password sign-in always works.
 *
 * Writes only static content — no LLM / network calls. Fully deterministic.
 *
 * Two ways to run:
 *   1. Automatically at server boot when `SEED_DEMO_ACCOUNT=1` (see server.ts).
 *      Idempotent: skips the heavy write if the account is already seeded,
 *      unless `force` is passed. This is how production gets seeded — set the
 *      Railway env var, redeploy, done. No SSH required.
 *   2. Manually via `bun run scripts/seed-demo-account.ts` (force reseed).
 *
 * Lives in src/ (not scripts/) so it ships in the Docker image and can be
 * imported by the server — scripts/ is excluded from some build contexts.
 */
import {
  copyFileSync,
  existsSync,
...
import { createUser, getUserByEmail, type User } from "./auth.ts";
import { getDb } from "./db.ts";
import { ensureUserDirs, userPaths, type UserPaths } from "./user-paths.ts";
import { withUserContext } from "./user-context.ts";
import { setProfileConfig } from "./user-settings.ts";

import type { AnalyzerResult, BillContact, BillingError } from "../types.ts";
import type { BonsaiReport, ThreadMessage } from "../orchestrator.ts";
import type { NegotiationState } from "../negotiate-email.ts";
import type { CallState } from "../voice/tool-handlers.ts";
import type { Baseline, OfferHuntResult, OfferRecord } from "../offer-agent.ts";

export const DEMO_EMAIL = "gcgeester04@gmail.com";
export const DEMO_PASSWORD = "BonsaiDemo!2026";

const __dirname = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = join(__dirname, "..", ".."); // src/lib → repo root
...
/**
 * Seed (or reseed) the demo account. Returns the resolved user id and whether
 * a heavy reseed actually happened.
 *
 * - `force: false` (default, used at server boot): if the account already has
 *   the seeded reports, return early without rewriting — keeps cold-start boots
 *   fast. Otherwise create/reuse the account, set the password, and write data.
 * - `force: true` (manual script run): always wipe and reseed.
 */
export async function seedDemoAccount(opts: { force?: boolean } = {}): Promise<{ userId: string; seeded: boolean }> {
  ...
  // Fast path for boot-time seeding: if the flagship report already exists and
  // we're not forcing, the account was seeded on a prior boot — nothing to do.
  if (!force && existsSync(P.reportPath("demo-medical-hospital"))) {
    return { userId: user.id, seeded: false };
  }
  ...
  console.log(`[seed] done. Account ${DEMO_EMAIL} seeded with ${bills.length} bills and ${comparisons.length} comparisons.`);
  return { userId: user.id, seeded: true };
}
