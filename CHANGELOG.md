...
- **Hunt poll cap dropped from 5 minutes to 90 seconds.** Real hunts that succeed almost always return inside 60s — keeping the spinner up for 5 minutes was a liveness lie. After 90s the page now flips to the regular Comparison chrome (banner hidden, "Recommended" / "All" filter chips, empty grid) instead of staying on the spinner. A new offer-hunt run on a subsequent audit (or a "Switch for me" click) clears the timed-out flag and re-polls normally.
